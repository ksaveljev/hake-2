...
glfwbKBDInit = do
  ...
  where
    windowError = error "GLFWbRenderer.glfwbKBDInit window is Nothing"
    kbdInit window = do
...
initKBDChanAndCallbacks window = do
    kbdChan <- io (newTChanIO :: IO (TChan GLFWKBDEvent))
    glfwbGlobals.glfwbKBDChan .= Just kbdChan
    io (initKBDCallbacks window kbdChan)

initKBDCallbacks :: GLFW.Window -> TChan GLFWKBDEvent -> IO ()
initKBDCallbacks window kbdChan = do
    GLFW.setKeyCallback window (Just (keyCallback kbdChan))
...
glfwbKBDUpdate = do
  ...
  where
    windowError = error "GLFWbRenderer.glfwbKBDUpdate window is Nothing"
    checkWindowClose window = do
        shouldClose <- io (GLFW.windowShouldClose window)
        when shouldClose $
...
glfwbKBDUninstallGrabs = do
  ...
  where
    windowError = error "GLFWbRenderer.glfwbKBDUninstallGrabs window is Nothing"
