...
checkOldMode Nothing = do
    monitor <- io GLFW.getPrimaryMonitor
...
createWindow monitor newDim fullScreen
    | fullScreen = do
        window <- io (GLFW.createWindow (newDim^._1) (newDim^._2) "Hake2 (GLFWb)" (Just monitor) Nothing)
        maybe createWindowError setWindow window
    | otherwise = do
        window <- io (GLFW.createWindow (newDim^._1) (newDim^._2) "Hake2 (GLFWb)" Nothing Nothing)
        maybe createWindowError setWindow window
  where
    createWindowError = Com.fatalError "GLFWbGLDriver createWindow returned Nothing"
    setWindow window = do
...
printFullScreenInfo monitor True = do
    videoMode <- io (GLFW.getVideoMode monitor)
    maybe videoModeError printVideoModeInfo videoMode
  where
    videoModeError = Com.fatalError "GLFWbGLDriver.GLFW.getVideoMode returned Nothing"
...
getModeString vm = B.concat
    [ encode (GLFW.videoModeWidth vm) , "x"
...
glDriverEndFrame = do
  ...
  where
    windowError = error "GLFWbGLDriver.glDriverEndFrame window is Nothing"
    swapBuf window = io (GLFW.swapBuffers window)
...
glDriverGetModeList = do
    oldMode <- use (glfwbGlobals.glfwbOldDisplayMode)
    monitor <- io GLFW.getPrimaryMonitor
    proceedGetModeList oldMode monitor
  where
    proceedGetModeList Nothing _ = do
        Com.fatalError "GLFWbGLDriver.glDriverGetModeList oldMode is Nothing"
        return V.empty
    proceedGetModeList _ Nothing = do
        Com.fatalError "GLFWbGLDriver.glDriverGetModeList monitor is Nothing"
        return V.empty
    proceedGetModeList (Just oldMode) (Just monitor) = do
