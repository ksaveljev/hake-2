...
glTexEnv mode = do
    ...
    when (mode /= fromIntegral lastMode) $ do
        request (GL.glTexEnvi GL.GL_TEXTURE_ENV GL.GL_TEXTURE_ENV_MODE (fromIntegral mode))
        setLastMode tmu
  where
    getLastMode tmu
  ...
  where
    ...
    proceedUpload colorTable palettedTexture
        | colorTable && (palettedTexture^.cvValue) /= 0 && isSky = do
            request (io (SV.unsafeWith image upload))
  ...
  where
    ...
    doCheckImage regSeq imageRef@(Ref idx) image
        | (image^.iRegistrationSequence) == regSeq || (image^.iRegistrationSequence) == 0 || (image^.iType) == Constants.itPic =
            return ()
        | otherwise = do
            request (deleteTexture (image^.iTexNum))
