...
deleteTexture :: Int -> Quake ()
deleteTexture texNum = io (with (fromIntegral texNum) (GL.glDeleteTextures 1))

getPalette :: Quake ()
getPalette = do
    imageData <- loadPCX "pics/colormap.pcx" True False
    proceedConstructPalette imageData
  where
    proceedConstructPalette Nothing = paletteError
...
constructPalette bs i =
    (255 `shiftL` 24) .|. (b `shiftL` 16) .|. (g `shiftL` 8) .|. r
  where
    j = i * 3
...
validPCX pcx
    ...
    | otherwise = True
  where
    badManufacturer = (pcx^.pcxManufacturer) /= 0x0A
...
loadPcxT (Just (fileHandle, len)) = do
    (res, _) <- parsePcxT
    either (const (return Nothing)) (return . Just) res
  where
    parsePcxT = runStateT (PB.decodeGet (getPcxT len)) (PBS.fromHandle fileHandle)

decodePCX :: Bool -> Bool -> PcxT -> (SV.Vector Word8, B.ByteString, (Int, Int))
decodePCX returnPalette returnDimensions pcx =
    (pix, palette, dimensions)
  where
    width = fromIntegral ((pcx^.pcxXMax) - (pcx^.pcxXMin) + 1)
...
doDecodePCX v raw inIdx outIdx x y width height
    ...
    | (raw `B.index` inIdx) .&. 0xC0 == 0xC0 = do
        let !runLength = fromIntegral ((raw `B.index` inIdx) .&. 0x3F)
    ...
    | otherwise = do
        MSV.write v outIdx (raw `B.index` inIdx)
        doDecodePCX v raw (inIdx + 1) (outIdx + 1) (x + 1) y width height
  where
    writeRunLength out len byte
        | len == 0 = return ()
        | otherwise = do
            MSV.write v out byte
...
loadColorTable True = do
    buf <- FS.loadFile "pics/16to8.dat"
    fastRenderAPIGlobals.frGLState.glsD16To8Table .= buf
    maybe colorTableError (const (return ())) buf
  where
    colorTableError = Com.fatalError "Couldn't load pics/16to8.pcx"
...
complexGamma gamma idx
  ...
  where
    inf = truncate (255 * (((fromIntegral idx + 0.5) / 255.5) ** gamma) + 0.5) :: Int
    inf' | inf < 0 = 0
         | inf > 255 = 255
         | otherwise = inf
...
genIntensityTable v idx
  ...
  where
    j = truncate (fromIntegral idx * v) :: Int
...
glTexEnv mode = do
    ...
    when (mode /= fromIntegral lastMode) $ do
        io (GL.glTexEnvi GL.GL_TEXTURE_ENV GL.GL_TEXTURE_ENV_MODE (fromIntegral mode))
        setLastMode tmu
  where
    getLastMode tmu
...
changeMipMap mode idx maxIdx
  ...
  where
    textureSettings image
        | (image^.iType) `notElem` [Constants.itPic, Constants.itSky] = do
            glBind (image^.iTexNum)
            io $ do
...
glBind texNum = do
  ...
  where
    pickTexNum _ Nothing = return texNum
    pickTexNum noBind (Just drawCharsRef)
        | (noBind^.cvValue) /= 0 = do
            img <- readRef drawCharsRef
            return (img^.iTexNum)
...
bindImage glState texNum
    ...
    | otherwise = do
        ...
        io (GL.glBindTexture GL.GL_TEXTURE_2D (fromIntegral texNum))
  where
    currentTexture
...
glLoadPic name pic width height picType bits = do
  ...
  where
    ...
    updateImageInfo idx hasAlpha = do
...
updateNumGLTextures numGLTextures idx
    | idx == numGLTextures = do
        checkError
        fastRenderAPIGlobals.frNumGLTextures += 1
    | otherwise = return ()
  where
    checkError | numGLTextures == Constants.maxGLTextures =
...
floodFillImage pic width height picType bits
    | picType == Constants.itSkin && bits == 8 = do
        d8to24table <- use (fastRenderAPIGlobals.frd8to24table)
        fifo <- use (fastRenderAPIGlobals.frFifo)
        io (MV.set fifo (0, 0))
        io (rFloodFillSkin pic width height d8to24table fifo)
...
rFloodFillSkin skin skinWidth skinHeight d8to24table fifo
    ...
    | otherwise = do
        skinMutable <- SV.unsafeThaw skin
  ...
  where
    fillColor = fromIntegral (SV.head skin)
...
doFloodFill skin skinWidth skinHeight fifo filledColor fillColor inpt outpt x y =
  ...
  where
    ...
    writeAndProceed (inpt4, fdc4) = do
        MSV.write skin pos fdc4
...
floodFillStep skin fifo inpt pos x y fillColor fdc off dx dy =
  ...
  where
    fillSkin :: Word8 -> IO (Int, Word8)
    fillSkin b
        | b == fillColor = do
            MSV.write skin (pos + off) 255
...
glUpload8 image width height mipmap isSky = do
  ...
  where
    ...
    sz = width * height
    proceedUpload colorTable palettedTexture
        | colorTable && (palettedTexture^.cvValue) /= 0 && isSky = do
            io (SV.unsafeWith image upload)
            filterMax <- use (fastRenderAPIGlobals.frGLFilterMax)
            io (glApplyFilters filterMax)
            return False
        | otherwise = do
            d8to24table <- use (fastRenderAPIGlobals.frd8to24table)
            glUpload32 (constructTrans image width d8to24table 0 sz) width height mipmap
    upload = GL.glTexImage2D GL.GL_TEXTURE_2D
                             0
...
constructTrans image width d8to24table idx sz = runST $ do
    v <- MSV.new (4 * sz)
...
glUpload32 image width height mipmap = do
  ...
  where
    !samples = scanAlpha image 0 3 (width * height)

calcScaledValue :: Int -> Bool -> Quake Int
calcScaledValue value mipmap = do
    roundDown <- glRoundDownCVar
...
getTextureComponents samples
    ...
    | otherwise = do
        VID.printf Constants.printAll (B.concat ["Unknown number of texture components ", encode samples, "\n"])
        return samples
...
uploadOrResample image width height mipmap scaledWidth scaledHeight samples comp
  ...
  where
    sameDimensions = scaledWidth == width && scaledHeight == height
...
glResampleTexture image width height scaledWidth scaledHeight =
  ...
  in runST $ do
         v <- MSV.new (4 * scaledWidth * scaledHeight)
...
glLightScaleTexture image width height onlyGamma = do
    gammaTable <- use (fastRenderAPIGlobals.frGammaTable)
    lightscale gammaTable
  where
    lightscale :: B.ByteString -> Quake (SV.Vector Word8)
    lightscale gammaTable
        | onlyGamma = return $ runST $ do
            v <- MSV.new (c * 4)
...
uploadMipMaps image scaledWidth scaledHeight mipLevel samples comp
  ...
  where
    scaled = glMipMap image scaledWidth scaledHeight
...
applyFilters mipmap = do
    ...
    filterMax <- use (fastRenderAPIGlobals.frGLFilterMax)
    io (proceedApplyFilters filterMin filterMax)
  where
    proceedApplyFilters filterMin filterMax
        | mipmap = glApplyFilters filterMin filterMax
        | otherwise = glApplyFilters filterMax filterMax
    glApplyFilters a b = do
        GL.glTexParameterf GL.GL_TEXTURE_2D GL.GL_TEXTURE_MIN_FILTER (fromIntegral a)
...
loadFromDisk imageName imageType
    ...
    | ".tga" `BC.isSuffixOf` imageName = do
        imageData <- loadTGA imageName
        maybe (return Nothing) loadTGAImage imageData
    | otherwise = do
        imageData <- loadPCX (B.concat ["pics/", imageName, ".pcx"]) False True
        maybe (return Nothing) loadPCXImage imageData
  where
    loadPCXImage (pic, _, (width, height)) =
...
uploadMiptexT name (Just miptex) =
  ...
  where
    fromBStoSV str = SV.generate (B.length str) (str `B.index`)

loadTGA :: B.ByteString -> Quake (Maybe (SV.Vector Word8, (Int, Int)))
loadTGA fileName =
    FS.fOpenFileWithLength fileName >>= loadTgaT >>= parseRawTGA
  where
    parseRawTGA Nothing = badTgaFile
    parseRawTGA (Just tga)
        | not (validTGA tga) = badTgaFile
        | otherwise = return (Just (decodeTGA tga))
    badTgaFile = do
        VID.printf Constants.printDeveloper (B.concat ["Bad tga file ", fileName, "\n"])
        return Nothing

validTGA :: TgaT -> Bool
validTGA tga
    | wrongImageType || wrongColorMap || wrongPixelSize = False
    | otherwise = True
  where
    wrongImageType = (tga^.tgaImageType) /= 2 && (tga^.tgaImageType) /= 10
    wrongColorMap = (tga^.tgaColorMapType) /= 0
    wrongPixelSize = (tga^.tgaPixelSize) /= 32 && (tga^.tgaPixelSize) /= 24

loadTgaT :: Maybe (Handle, Int) -> Quake (Maybe TgaT)
loadTgaT Nothing = return Nothing
loadTgaT (Just (fileHandle, len)) = do
    (res, _) <- parseTgaT
    either (const (return Nothing)) (return . Just) res
  where
    parseTgaT = runStateT (PB.decodeGet (getTgaT len)) (PBS.fromHandle fileHandle)

decodeTGA :: TgaT -> (SV.Vector Word8, (Int, Int))
decodeTGA tga
    | (tga^.tgaImageType) == 2 = runST $ do
        pic <- MSV.new (columns * rows * 4)
        pic' <- readUncompressedTGA buf pic (tga^.tgaPixelSize) 0 (rows - 1) 0 columns
        return (pic', (rows, columns))
    | otherwise = error "Image.decodeTGA [image type == 10]"
  where
    columns = fromIntegral (tga^.tgaWidth)
    rows = fromIntegral (tga^.tgaHeight)
    buf | (tga^.tgaIdLength) /= 0 = B.drop (fromIntegral (tga^.tgaIdLength)) (tga^.tgaData)
        | otherwise = tga^.tgaData

readUncompressedTGA :: B.ByteString -> MSV.STVector s Word8 -> Word8 -> Int -> Int -> Int -> Int -> ST s (SV.Vector Word8)
readUncompressedTGA buf pic pixelSize idx row minRow columns
    | row < minRow = SV.unsafeFreeze pic
    | otherwise = do
        idx' <- readUncompressedTGARow buf pic pixelSize idx pixbuf 0 columns
        readUncompressedTGA buf pic pixelSize idx' (row - 1) minRow columns
  where
    pixbuf = row * columns * 4

readUncompressedTGARow :: B.ByteString -> MSV.STVector s Word8 -> Word8 -> Int -> Int -> Int -> Int -> ST s Int
  ...
  where
    ...
    doCheckImage regSeq imageRef@(Ref idx) image
        | (image^.iRegistrationSequence) == regSeq || (image^.iRegistrationSequence) == 0 || (image^.iType) == Constants.itPic =
            return ()
        | otherwise = do
            deleteTexture (image^.iTexNum)
            writeRef imageRef (newImageT idx)
