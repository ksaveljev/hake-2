...
fastInit glDriver _ _ = do
    VID.printf Constants.printAll (B.concat ["ref_gl version: ", Constants.refVersion, "\n"])
    Image.getPalette
    rRegister
    fastRenderAPIGlobals.frGLState.glsPrevMode .= 3
    rSetMode glDriver >>= initResult
  where
    initResult True = return True
    initResult False = do
...
identifyGLRenderer = do
    renderer <- fmap (BC.map toLower) (use (fastRenderAPIGlobals.frGLConfig.glcRendererString))
  ...
  where
    identifyRenderer renderer vendor
...
rendererSpecificSettings rendererId = do
    monoLightSettings rendererId =<< glMonoLightMapCVar
  ...
  where
    setDrawAll | rendererId .&. Constants.glRendererPowerVR /= 0 = "1"
               | otherwise = "0"

monoLightSettings :: Int -> CVarT -> Quake ()
monoLightSettings rendererId monoLightMap
    | useMlm && rendererId == Constants.glRendererPerMedia2 = do
...
checkCompiledVertexArray extensions = do
    enabled <- checkEnabled
    fastRenderAPIGlobals.frLockArraysEXT .= enabled
  where
    checkEnabled
        | extensionAvailable = do
...
checkSwapControl extensions = do
    enabled <- checkEnabled
...
checkPointParameters extensions = do
    enabled <- checkEnabled
...
checkMultiTexture extensions = do
    enabled <- checkEnabled =<< checkExtensionWorks
    fastRenderAPIGlobals.frActiveTextureARB .= enabled
  where
    checkEnabled ok
        | extensionAvailable && ok = do
        ...
        | otherwise = do
            ...
            return False
    checkExtensionWorks = io execActiveTexture
    execActiveTexture = handle (\(_ :: IOException) -> return False) $ do
        GL.glClientActiveTextureARB (fromIntegral Constants.glTexture0ARB)
        return True
...
finishInit glDriver enabled
    | not enabled && not True = do -- TODO: remove me, used for testing with dummy gldriver
...
getGLString n = do
    str <- GL.glGetString n
    maybeNullPtr (return B.empty) (B.packCString . castPtr) str
  where
    maybeNullPtr nothing f ptr | ptr == nullPtr = nothing
                               | otherwise = f ptr
...
setLightLevel newRefDef
    | (newRefDef^.rdRdFlags) .&. Constants.rdfNoWorldModel == 0 = do
...
fastSetPalette maybePalette = do
    maybe noPalette generatePalette maybePalette
...
fastBeginFrame glDriver cameraSeparation = do
    updateVidDef =<< use (fastRenderAPIGlobals.frVid)
...
changeModeIfNeeded fullScreen glMode
    | (fullScreen^.cvModified) || (glMode^.cvModified) = do
        ref <- CVar.get "vid_ref" "GLFWb" 0
        maybe vidRefError updateVar ref
    | otherwise = return ()
  where
    vidRefError = Com.fatalError "FastRenderAPI.changeModeIfNeeded ref is Nothing"
    updateVar ref = CVar.update (ref & cvModified .~ True)

checkGLLog :: GLDriver -> CVarT -> Quake ()
checkGLLog glDriver glLog = do
    when (glLog^.cvModified) $ do
        (glDriver^.gldEnableLogging) logEnabled
        CVar.update (glLog & cvModified .~ False)
    when logEnabled (glDriver^.gldLogNewFrame)
  where
    logEnabled = (glLog^.cvValue) /= 0

checkVidGamma :: CVarT -> Quake ()
checkVidGamma vidGamma
    | vidGamma^.cvModified = do
        CVar.update (vidGamma & cvModified .~ False)
...
checkTextureAlphaMode textureAlphaMode
    | textureAlphaMode^.cvModified = do
...
checkTextureSolidMode textureSolidMode
    | textureSolidMode^.cvModified = do
...
setMode glDriver fullScreen glMode vid = do
...
checkFullScreenMode glDriver dim glMode err
    | err == Constants.rsErrInvalidFullscreen = do
...
tryWithoutFullScreen glDriver dim glMode = do
...
revertModeIfNeeded glDriver dim err False = do
...
checkInvalidMode prevMode err
    | err == Constants.rsErrInvalidMode = do
...
tryPrevMode glDriver dim prevMode = do
...
setImageTextureModes = do
    Image.glTextureMode =<< fmap (^.cvString) glTextureModeCVar
...
setTextureFilterWrapBlend = do
    minFilter <- use (fastRenderAPIGlobals.frGLFilterMin)
    maxFilter <- use (fastRenderAPIGlobals.frGLFilterMax)
    io (setGLParams minFilter maxFilter)
  where
    setGLParams minFilter maxFilter = do
...
pointParameterSetting True = do
    a <- fmap (^.cvValue) glParticleAttACVar
    ...
    io (ppExt a b c minSize maxSize)
  where
    ppExt a b c minSize maxSize = do
...
vertexArraySettings texture0 = io $ do
...
rInitParticleTexture = do
    pt <- Image.glLoadPic "***particle***" particleTexture 8 8 Constants.itSprite 32
...
glUpdateSwapInterval glDriver =
  ...
  where
    updateSwapInterval swapInterval
        | swapInterval^.cvModified = do
            CVar.update (swapInterval & cvModified .~ False)
...
rClear ztrick clear
    | (ztrick^.cvValue) /= 0 = do
        when ((clear^.cvValue) /= 0) $
            io (GL.glClear GL.GL_COLOR_BUFFER_BIT)
    ...
    | otherwise = do
        ...
        io (GL.glDepthFunc GL.GL_LEQUAL)
        setDepthRange
...
checkTrickFrame trickFrame
    ...
    | otherwise = do
        ...
        io (GL.glDepthFunc GL.GL_GEQUAL)

checkClear :: CVarT -> Quake ()
checkClear clear
    | (clear^.cvValue) /= 0 =
        io (GL.glClear (GL.GL_COLOR_BUFFER_BIT .|. GL.GL_DEPTH_BUFFER_BIT))
    | otherwise =
        io (GL.glClear GL.GL_DEPTH_BUFFER_BIT)

setDepthRange :: Quake ()
setDepthRange = do
    depthMin <- use (fastRenderAPIGlobals.frGLDepthMin)
    depthMax <- use (fastRenderAPIGlobals.frGLDepthMax)
    io (GL.glDepthRange (realToFrac depthMin) (realToFrac depthMax))
...
rSetFrustum = do
  ...
  where
    calcNormals vUp vRight vPn newRefDef = V.fromList
...
rSetupGL = do
    ...
    vid <- use (fastRenderAPIGlobals.frVid)
    io $ do
        GL.glViewport (fromIntegral (newRefDef^.rdX))
  ...
  where
    setCullFace glCull
        | (glCull^.cvValue) /= 0 = io (GL.glEnable GL.GL_CULL_FACE)
        | otherwise = io (GL.glDisable GL.GL_CULL_FACE)
...
rDrawEntitiesOnList = do
    ...
    unless (drawEntities == 0) $ do
        ...
        mapM_ drawNonTransparentEntity (newRefDef^.rdEntities) -- TODO: make sure number of rdEntities is the same as newRefDef^.rdNumEntities
...
drawNonTransparentEntity entityRef = do
...
rDrawParticles = do
    extPointParameters <- glExtPointParametersCVar
...
drawParticle sourceVertices sourceColors up right vpn origin idx = do
...
setCurrentViewCluster newRefDef (Just worldModelRef) = do
    ...
    leaf <- io (Model.pointInLeaf (newRefDef^.rdViewOrg) worldModel)
  ...
  where
    proceedSetCurrentViewCluster worldModel leaf
        | (leaf^.mlContents) == 0 = do
            leaf' <- io (Model.pointInLeaf ((newRefDef^.rdViewOrg) & _z -~ 16) worldModel)
...
rDrawBeam = error "FastRenderAPI.rDrawBeam" -- TODO
