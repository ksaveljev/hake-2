...
drawAliasModel :: Ref RefDefT EntityT -> Ref' ModelT -> Quake ()
drawAliasModel entityRef currentModelRef = do
  ...
  where
    doDrawAliasModel currentModel shadeLight (Just (AliasModelExtra pAliasHdr)) = do
        ...
        updatedCurrentEntity <- readCurrentEntity currentEntityRef
        ...
        when ((updatedCurrentEntity^.enFlags) .&. Constants.rfTranslucent /= 0) $
            request (GL.glDisable GL.GL_BLEND)
    ...
    checkCurrentEntityFrame currentEntityRef pAliasHdr = do
        ...
        when ((currentEntity^.eFrame) >= (pAliasHdr^.dmNumFrames) || (currentEntity^.eFrame) < 0) $ do
            ...
            modifyCurrentEntity currentEntityRef (\v -> v & eFrame .~ 0
                                                          & eOldFrame .~ 0)
    ...
    checkLerpModels currentEntityRef = do
        ...
        when (lerpModels == 0) $
            modifyCurrentEntity currentEntityRef (\v -> v & eBackLerp .~ 0)

buildShadeLight :: Maybe CurrentEntity -> Quake (V3 Float)
...
rCullAliasModel :: Ref RefDefT EntityT -> Quake Bool
...
cullAliasModel :: Ref RefDefT EntityT -> ModelT -> Maybe ModelExtra -> Quake Bool
cullAliasModel entRef currentModel (Just (AliasModelExtra pAliasHdr)) = do
    checkFrames =<< readRef entRef
  ...
  where
    checkFrames e = do
        ...
        when ((e^.eOldFrame) >= (pAliasHdr^.dmNumFrames) || (e^.eOldFrame) < 0) $ do
            ...
            modifyRef entRef (\v -> v & eOldFrame .~ 0)
...
glDrawAliasFrameLerp pAliasHdr backLerp shadeLight shadeDots = do
    ...
    request $ do
        textureArrayBuf <- use frTextureArrayBuf
        srcTextureCoords <- use frModelTextureCoordBuf
        vertexIndexBuf <- use frModelVertexIndexBuf
        ...
        io (drawElements textureArrayBuf vertexIndexBuf srcTextureCoords 0 0 (UV.length (pAliasHdr^.dmCounts)))
        when ((currentEntity^.enFlags) .&. (Constants.rfShellRed .|. Constants.rfShellGreen .|. Constants.rfShellBlue .|. Constants.rfShellDouble .|. Constants.rfShellHalfDam) /= 0) $
            GL.glEnable GL.GL_TEXTURE_2D
  ...
  where
    ...
    addShellColor currentEntity frame alpha
        ...
        | otherwise = do
            request $ do
                colorArrayBuf <- use frColorArrayBuf
