...
rSetCacheState surfRef =
  do surf <- readRef surfRef
     newRefDef <- use (fastRenderAPIGlobals.frNewRefDef)
     return $ runST $
       do lights <- UV.unsafeThaw (surf^.msCachedLight)
...
cacheLights newRefDef surf lights idx maxIdx
  ...
  | style == 255 = return ()
  | otherwise =
      do MUV.write lights idx (((newRefDef^.rdLightStyles) V.! (fromIntegral style))^.lsWhite)
         cacheLights newRefDef surf lights (idx + 1) maxIdx
  where style = (surf^.msStyles) `B.index` idx
