...
updateImageRegSeq :: Int -> IORef MTexInfoT -> Quake ()
updateImageRegSeq regSeq texInfoRef = do
...
loadModel modKnown name crash = do
    io $ B.putStrLn ("LOAD MODEL FFS" `B.append` name)
...
loadPlanes loadModelRef buf lump = do
    checkLump
    planes <- io (V.mapM toCPlane getPlanes)
...
toCPlane dPlane = newIORef CPlaneT
...
loadTexInfo loadModelRef buf lump = do
    checkLump
    mti <- io (V.replicateM count (newIORef undefined)) -- TODO: hacky, maybe newIORef newMTexInfoT ?
    V.imapM_ (toMTexInfoT mti) readTexInfo
    io (V.mapM_ countAnimationFrames mti)
    modifyRef loadModelRef (\v -> v { _mNumTexInfo = count
                                    , _mTexInfo = mti
                                    })
  where
    checkLump =
...
toMTexInfoT :: V.Vector (IORef MTexInfoT) -> Int -> TexInfoT -> Quake ()
toMTexInfoT mti idx texInfo = do
    foundImage <- Image.glFindImage name Constants.itWall
    imageRef <- maybe returnNoTexture return foundImage
    io $ writeIORef (mti V.! idx) MTexInfoT
        { _mtiVecs = texInfo^.tiVecs
        , _mtiFlags = texInfo^.tiFlags
        , _mtiNumFrames = 1
        , _mtiNext = if (texInfo^.tiNextTexInfo) > 0 then Just (mti V.! (texInfo^.tiNextTexInfo)) else Nothing
...
countAnimationFrames :: IORef MTexInfoT -> IO ()
countAnimationFrames mTexInfoRef = do
...
countFrames :: IORef MTexInfoT -> Maybe (IORef MTexInfoT) -> Int -> IO Int
countFrames _ Nothing n = return n
countFrames initialRef (Just currentRef) n
    | currentRef == initialRef = return n
    | otherwise = do
        texInfo <- io (readIORef currentRef)
        countFrames initialRef (texInfo^.mtiNext) (n + 1)
...
loadFaces loadModelRef buf lump = do
    ...
    surfaces <- V.mapM (constructMSurfaceT loadModelRef) dFaces
...
constructMSurfaceT :: Ref ModelT -> DFaceT -> Quake (IORef MSurfaceT)
constructMSurfaceT loadModelRef dFace = do
    model <- readRef loadModelRef
    when (ti < 0 || ti >= (model^.mNumTexInfo)) $
        Com.comError Constants.errDrop "MOD_LoadBmodel: bad texinfo number"
    surfaceRef <- io $ newIORef $ newMSurfaceT & msFirstEdge .~ dFace^.dfFirstEdge
                                            & msNumEdges .~ fromIntegral (dFace^.dfNumEdges)
                                            & msFlags .~ (if (dFace^.dfSide) /= 0 then Constants.surfPlaneBack else 0)
                                            & msPolys .~ Nothing
                                            & msPlane .~ Just ((model^.mPlanes) V.! planeNum)
                                            & msTexInfo .~ (model^.mTexInfo) V.! ti
                                            & msStyles .~ dFace^.dfStyles -- TODO: should we limit it by Constants.maxLightMaps ?
                                            & msSamples .~ getSamples (model^.mLightdata)
    io (calcSurfaceExtents model surfaceRef)
  ...
  where
    ...
    planeNum = fromIntegral (dFace^.dfPlaneNum)
    getSamples Nothing = Nothing
    getSamples (Just lightData)
        | (dFace^.dfLightOfs) == -1 = Nothing
        | otherwise = Just (B.drop (dFace^.dfLightOfs) lightData)

calcSurfaceExtents :: ModelT -> IORef MSurfaceT -> IO ()
calcSurfaceExtents model surfaceRef = do
    surface <- readIORef surfaceRef
    texInfo <- readIORef (surface^.msTexInfo)
    ...
    modifyIORef' surfaceRef (\v -> v & msTextureMins .~ mapTuple (* 16) bmins
...
createWarps :: IORef MSurfaceT -> Quake ()
createWarps surfaceRef = do
    surface <- io (readIORef surfaceRef)
    texInfo <- io (readIORef (surface^.msTexInfo))
    when ((texInfo^.mtiFlags) .&. Constants.surfWarp /= 0) $ do
        io $ modifyIORef' surfaceRef (\v -> v & msFlags %~ (.|. Constants.surfDrawTurb)
                                              & msExtents .~ (16384, 16384)
                                              & msTextureMins .~ (-8192, -8192))
        Warp.glSubdivideSurface surfaceRef

createLightmaps :: IORef MSurfaceT -> Quake ()
createLightmaps surfaceRef = do
    surface <- io (readIORef surfaceRef)
    texInfo <- io (readIORef (surface^.msTexInfo))
...
createPolygons :: IORef MSurfaceT -> Quake ()
...
loadMarkSurfaces loadModelRef buf lump = do
    checkLump
    model <- readRef loadModelRef
    markSurfaces <- V.mapM ((toMSurfaceT model) . fromIntegral) getMarkSurfaces
  ...
  where
    ...
    toMSurfaceT model idx = do
        when (idx < 0 || idx >= (model^.mNumSurfaces)) $
...
loadLeafs loadModelRef buf lump = do
    checkLump
    leafs <- io (V.mapM (newIORef . toMLeaf) getLeafs)
...
loadNodes loadModelRef buf lump = do
    ...
    V.imapM_ (toMNode model nodes) getNodes
    modifyRef loadModelRef (\v -> v & mNumNodes .~ count
                                    & mNodes .~ nodes)
    io (setParent (MNodeChildRef (nodes V.! 0)) Nothing) -- sets nodes and leafs
  where
...
toMNode :: ModelT -> V.Vector (IORef MNodeT) -> Int -> DNodeT -> Quake ()
toMNode model nodes idx dNode =
    io $ writeIORef (nodes V.! idx) MNodeT
        { _mnContents     = (-1) -- differentiate from leafs
        , _mnVisFrame     = 0
        , _mnMins         = fmap fromIntegral (dNode^.dnMins)
        , _mnMaxs         = fmap fromIntegral (dNode^.dnMaxs)
        , _mnParent       = Nothing
        , _mnPlane        = (model^.mPlanes) V.! (dNode^.dnPlaneNum)
        , _mnChildren     = getChildren model nodes (dNode^.dnChildren)
        , _mnFirstSurface = fromIntegral (dNode^.dnFirstFace)
        , _mnNumSurfaces  = fromIntegral (dNode^.dnNumFaces)
        }

getChildren :: ModelT -> V.Vector (IORef MNodeT) -> (Int, Int) -> (MNodeChild, MNodeChild)
getChildren model nodes (p1, p2) =
          let a = if p1 >= 0
                    then MNodeChildRef (nodes V.! p1)
                    else MLeafChildRef ((model^.mLeafs) V.! ((-1) - p1))
              b = if p2 >= 0
                    then MNodeChildRef (nodes V.! p2)
                    else MLeafChildRef ((model^.mLeafs) V.! ((-1) - p2))
          in (a, b)

setParent :: MNodeChild -> Maybe (IORef MNodeT) -> IO ()
setParent (MNodeChildRef nodeRef) parent = do
    modifyIORef' nodeRef (\v -> v & mnParent .~ parent)
    node <- readIORef nodeRef
    setParent (node^.mnChildren._1) (Just nodeRef)
    setParent (node^.mnChildren._2) (Just nodeRef)
setParent (MLeafChildRef leafRef) parent = do
    modifyIORef' leafRef (\v -> v & mlParent .~ parent)
...
precompileGLCmds model = do
    ...
    (tmp, modelTextureCoordIdx', modelVertexIndexIdx') <- do
...
setTextureAndVertex :: MSV.IOVector Float -> MSV.IOVector Int32 -> UV.Vector Word32 -> Int -> Int -> Int -> [Int32] -> Quake ([Int32], Int, Int)
...
setCoords textureBuf vertexBuf order textureCoordIdx vertexIndexIdx orderIndex count
    ...
    | otherwise = do
        io $ do
            MSV.write textureBuf textureCoordIdx (wordToFloat (order UV.! orderIndex))
            MSV.write textureBuf (textureCoordIdx + 1) (wordToFloat (order UV.! (orderIndex + 1)))
            MSV.write vertexBuf vertexIndexIdx (fromIntegral (order UV.! (orderIndex + 2)))
...
pointInLeaf :: V3 Float -> ModelT -> IO MLeafT
pointInLeaf p model =
    findLeaf (MNodeChildRef ((model^.mNodes) V.! 0))
  where
    findLeaf (MNodeChildRef nodeRef) = do
        node <- readIORef nodeRef
        plane <- readIORef (node^.mnPlane)
        let d = p `dot` (plane^.cpNormal) - (plane^.cpDist)
            childRef
                | d > 0 = node^.mnChildren._1
                | otherwise = node^.mnChildren._2
        case childRef of
            MLeafChildRef leafRef -> readIORef leafRef
