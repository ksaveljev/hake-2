                        ...
                        then return True
                          ...
                          if isJust mask
                            then do
                              ...
                              orig <- readRef edictRef >>= \e -> return (e^.eEntityState.esOrigin)
...
startSound maybeOrigin edictRef channel soundIndex volume attenuation timeOfs = do
    ...
    ent <- readRef edictRef >>= \e -> return (e^.eIndex)
...
sendClientDatagram clientRef@(Ref clientIdx) = do
    ...
    io $ print "SVSend.sendClientDatagram"
        ...
        sendMessage clientRef@(Ref clientIdx) = do
          ...
          when ((client^.cState) /= 0) $ do
            ...
            if | elem state [Constants.ssCinematic, Constants.ssDemo, Constants.ssPic] -> do
                   ...
                   io $ print "SVSend.sendClientMessages #1"
