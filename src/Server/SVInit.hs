...
extractLevelAndNextServer levelString = maybe noNextServer setNextServer splitIdx
  where
    splitIdx = '+' `BC.elemIndex` levelString
    noNextServer = do
        void (CVar.set "nextserver" B.empty)
        return levelString
    setNextServer idx = do
        void (CVar.set "nextserver" (B.concat ["gamemap \"", B.drop (idx + 1) levelString, "\""]))
...
spawnLevelAndReconnect level attractLoop loadGame = do
  ...
  where
    (spawnPoint, updatedLevel) = checkSpawnPoint ('$' `BC.elemIndex` level)
...
initGame :: Quake ()
initGame = do
    restartServer =<< use (svGlobals.svServerStatic.ssInitialized)
    CVar.getLatchedVars
...
restartServer False = do
    CL.dropClient
    SCR.beginLoadingPlaque

checkCoopDeathmatch :: Quake ()
checkCoopDeathmatch = do
...
setServerStaticSettings = do
    r <- Lib.rand
    maxClients <- fmap (truncate . (^.cvValue)) maxClientsCVar
    svGlobals.svServerStatic.ssSpawnCount .= fromIntegral r
    svGlobals.svServerStatic.ssClients .= V.generate maxClients (\idx -> newClientT & cServerIndex .~ idx)
    svGlobals.svServerStatic.ssNumClientEntities .= maxClients * Constants.updateBackup * 64
    svGlobals.svServerStatic.ssClientEntities .= V.replicate (maxClients * Constants.updateBackup * 64) (newEntityStateT Nothing)

initHeartbeat :: Quake ()
initHeartbeat = do
    svGlobals.svServerStatic.ssLastHeartbeat .= -99999
  ...
  where
    netAdrError = error "SVInit.initHeartbeat idMasterNetAdr is Nothing"
    setMasterAdr adr = svGlobals.svMasterAdr %= (V.// [(0, adr)])

resetClients :: Quake ()
resetClients = svGlobals.svServerStatic.ssClients %= V.imap resetClient
  where
    resetClient idx client = client & cEdict .~ Just (Ref (idx + 1))
...
spawnServer server spawnPoint srvState attractLoop loadGame = do
    when attractLoop (void (CVar.set "paused" "0"))
...
spawnServerUpdateState server attractLoop loadGame = do
...
spawnServerDeathmatchBasedSettings deathmatch
    | deathmatch /= 0 = do
        airAccelerate <- fmap (^.cvValue) svAirAccelerateCVar
...
initServerClientsAndModels server srvState = do
  ...
  where
    loadContent | srvState /= Constants.ssGame = CM.loadMap B.empty False [0] -- no real map
                | otherwise = do
...
updateServerModels = do
  ...
  where
    updateModelsConfigStrings idx = (Constants.csModels + 1 + idx, "*" `B.append` encode idx) 
...
spawnMapEntities server spawnPoint srvState = do
...
saveEdictEntityState edictRef@(Ref idx) = do
    edict <- readRef edictRef
    svGlobals.svServer.sBaselines.ix idx .= (edict^.eEntityState)

checkForSavegame :: Quake ()
checkForSavegame = io (putStrLn "SVInit.checkForSavegame IMPLEMENT ME!") -- TODO