...
import           Control.Monad.State          (State, StateT, MonadState, MonadIO)
import           Control.Monad.State          (lift, get, put, liftIO)
...
data IORequest x = forall a. RunIO (QuakeIO a) (a -> x)
...
data FastRenderAPIGlobals = FastRenderAPIGlobals
    ...
    , _frFrameCount           :: Int
    , _frWarpFace             :: Maybe (Ref' MSurfaceT)
...
data CurrentEntity = VEntityRef (Ref VGlobals EntityT)
                   | RDEntityRef (Ref RefDefT EntityT)
                   | NewEntity EntityT
