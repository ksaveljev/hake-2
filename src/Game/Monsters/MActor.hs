...
spMiscActor selfRef =
  ...
  where proceedSpawnMiscActor self Nothing _ gameImport _ =
          do (gameImport^.giDprintf) (B.concat [self^.eClassName, " with no target at ", Lib.vtos (self^.eEntityState.esOrigin), "\n"])
             GameUtil.freeEdict selfRef
        proceedSpawnMiscActor self _ Nothing gameImport _ =
          do (gameImport^.giDprintf) (B.concat ["untargeted ", self^.eClassName, " at ", Lib.vtos (self^.eEntityState.esOrigin), "\n"])
             GameUtil.freeEdict selfRef
        proceedSpawnMiscActor _ _ _ gameImport deathmatch
          | (deathmatch^.cvValue) /= 0 = GameUtil.freeEdict selfRef
          | otherwise =
              do modelIdx <- (gameImport^.giModelIndex) (Just "players/male/tris.md2")
...
spTargetActor selfRef =
  do self <- readRef selfRef
     ...
     (gameImport^.giLinkEntity) selfRef
  where checkTargetName self Nothing gameImport =
...
checkTargetSpawnFlags selfRef self
  | (self^.eSpawnFlags) .&. 1 /= 0 =
      do gameBaseGlobals.gbSpawnTemp.stHeight %= (\v -> if v == 0 then 200 else v)
...
doActorStand selfRef =
  do modifyRef selfRef (\v -> v & eMonsterInfo.miCurrentMove .~ Just actorMoveStand)
     randomizeOnStartup =<< use (gameBaseGlobals.gbLevel.llTime)
     return True
  where randomizeOnStartup levelTime
          | levelTime < 1.0 =
              do r <- Lib.rand
...
actorWalk :: EntThink
actorWalk = EntThink "actor_walk" $ \selfRef ->
  do modifyRef selfRef (\v -> v & eMonsterInfo.miCurrentMove .~ Just actorMoveWalk)
     return True
...
actorFramesWalk =
  ...
  where dists = [0, 6, 10, 3, 2, 7, 10, 1, 4, 0, 0]
...
doActorRun selfRef =
  do self <- readRef selfRef
     ...
     return True
  where runBasedOnTarget self levelTime
...
actorFramesRun =
  ...
  where dists = [4, 15, 15, 8, 20, 15, 8, 17, 12, -2, -2, -1]
...
doActorFire selfRef =
  do actorMachineGun selfRef
     levelTime <- use (gameBaseGlobals.gbLevel.llTime)
     self <- readRef selfRef
     checkPause self levelTime
     return True
  where checkPause self levelTime
