...
import           Game.MonsterInfoT (newMonsterInfoT)
import           Game.MoveInfoT (newMoveInfoT)
import           Types
...
newEdictT idx =
  EdictT { _eEntityState           = newEntityStateT (Just (EdictRef idx))
         , _eInUse                 = False
         , _eClassName             = ""
         , _eLinkCount             = 0
         , _eArea                  = LinkRef (2 * Constants.areaNodes + idx)
         , _eNumClusters           = 0
