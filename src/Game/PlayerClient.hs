...
clientConnect edictRef userInfo =
  do value <- Info.valueForKey userInfo "ip"
     ...
     checkBanned banned
  where checkBanned banned
          | banned =
              do userInfo' <- Info.setValueForKey userInfo "rejmsg" "Banned."
                 return (False, userInfo')
          | otherwise =
              do value <- Info.valueForKey userInfo "spectator"
        ...
        checkSpectator value deathmatch
          ...
          | otherwise =
              do password <- Info.valueForKey userInfo "password"
                 actualPassword <- fmap (^.cvString) spectatorPasswordCVar
                 verifySpectatorPassword password actualPassword
        verifySpectatorPassword password actualPassword
          | not (passwordOK actualPassword password) =
              do userInfo' <- Info.setValueForKey userInfo "rejmsg" "Password required or incorrect."
...
doClientConnect edictRef@(Ref _ idx) userInfo False =
  ...
  where gClientRef = Ref Constants.noParent (idx - 1)
        ...
        checkMaxClients gClient maxClients
          | maxClients > 1 =
              do dprintf <- use (gameBaseGlobals.gbGameImport.giDprintf)
...
clientUserInfoChanged edictRef userInfo
  ...
  where gClientRefError =
          do Com.fatalError "PlayerClient.clientUserInfoChanged edict^.eClient is Nothing"
             return badInfoString
        badInfoString = "\\name\\badinfo\\skin\\male/grunt"
        proceedClientUserInfoChanged edict gClientRef =
          do name <- Info.valueForKey userInfo "name"
             deathmatch <- deathmatchCVar
             ...
             return userInfo
        checkSpectator gClientRef deathmatch =
          do str <- Info.valueForKey userInfo "spectator"
             modifyRef gClientRef (\v -> v & gcPers.cpSpectator .~ ((deathmatch^.cvValue) /= 0 && str /= "0"))
        setSkin name playerNum =
          do skin <- Info.valueForKey userInfo "skin"
