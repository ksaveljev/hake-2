...
pickupArmorF edictRef otherRef = do
...
proceedPickupArmor edictRef edict otherRef (Just gClientRef) (Just gItemRef) = do
  ...
  where
    ...
    applyDeathmatchActions True dmv
        | (edict^.eSpawnFlags) .&. Constants.droppedItem == 0 && dmv /= 0 = do
...
applyNewArmor gClientRef gClient gItem oldInfo newInfo oldArmorIndex = do
    modifyRef gClientRef (\v -> v & gcPers.cpInventory.ix oldArmorIndex .~ 0
                                  & gcPers.cpInventory.ix (gItem^.giIndex) .~ newCount)
    return True
  where
    salvage = (oldInfo^.giaNormalProtection) / (newInfo^.giaNormalProtection)
...
updateOldArmor gClientRef gClient oldInfo newInfo oldArmorIndex
    ...
    | otherwise = do
        modifyRef gClientRef (\v -> v & gcPers.cpInventory.ix oldArmorIndex .~ newCount)
        return True
  where
    currentCount = (gClient^.gcPers.cpInventory) UV.! oldArmorIndex
...
pickupPowerArmorF edictRef otherRef = do
    edict <- readRef edictRef
    other <- readRef otherRef
...
proceedPickupPowerArmor edictRef edict otherRef (Just gClientRef) (Just gItemRef) = do
    gItem <- readRef gItemRef
...
proceedUsePowerArmor edictRef edict gameImport
    | (edict^.eFlags) .&. Constants.flPowerArmor /= 0 = do
...
checkCellsForPowerArmor edictRef gameImport (Just cellsRef) (Just gClientRef) = do
    cells <- readRef cellsRef
    gClient <- readRef gClientRef
...
dropPowerArmorF edictRef gItemRef = do
    edict <- readRef edictRef
...
proceedDropPowerArmor edictRef edict gItemRef (Just gClientRef) = do
    gClient <- readRef gClientRef
    gItem <- readRef gItemRef
...
pickupAmmoF edictRef otherRef = do
    edict <- readRef edictRef
    other <- readRef otherRef
...
calcAndAddAmmo edictRef edict otherRef gClientRef gClient gItemRef gItem dmfv = do
  ...
  where
    ...
    equipWeaponAndSetRespawn True = do
        dmv <- deathmatchValue
        equipBlaster dmv
    ...
    deathmatchValue = fmap (^.cvValue) deathmatchCVar
    equipBlaster dmv
        | isWeapon && oldCount == 0 = do
            blasterRef <- findItem "blaster"
...
dropAmmoF edictRef gItemRef = do
    gItem <- readRef gItemRef
...
calcAndDropAmmo edictRef gItem droppedEdictRef gClientRef gClient = do
    modifyRef droppedEdictRef (\v -> v & eCount .~ count)
  ...
  where
    count | currentAmmoCount >= (gItem^.giQuantity) = gItem^.giQuantity
          | otherwise = currentAmmoCount
    currentAmmoCount = (gClient^.gcPers.cpInventory) UV.! (gItem^.giIndex)
    finishDropAmmo droppedEdict = do
        modifyRef gClientRef (\v -> v & gcPers.cpInventory.ix (gItem^.giIndex) -~ (droppedEdict^.eCount))
        GameUtil.validateSelectedItem edictRef -- RESEARCH: why does jake2 has same validate method in Cmd (which is used here)
    checkIfCanDropWeapon droppedEdict weaponRef = do
        weapon <- readRef weaponRef
        dropWeaponOrAmmo droppedEdict weapon
    dropWeaponOrAmmo droppedEdict weapon
        | (weapon^.giTag) == Constants.ammoGrenades && (gItem^.giTag) == Constants.ammoGrenades && ((gClient^.gcPers.cpInventory) UV.! (gItem^.giIndex)) - (droppedEdict^.eCount) <= 0 = do
...
proceedUseQuad edictRef gItem (Just gClientRef) = do
    modifyRef gClientRef (\v -> v & gcPers.cpInventory.ix (gItem^.giIndex) -~ 1)
...
pickupPowerupF edictRef otherRef = do
    edict <- readRef edictRef
    other <- readRef otherRef
...
proceedPickupPowerup edictRef edict otherRef (Just gClientRef) (Just gItemRef) = do
    gItem <- readRef gItemRef
    gClient <- readRef gClientRef
...
verifyAndPickupPowerup edictRef edict otherRef gClientRef gClient gItemRef gItem skillValue coopValue
    ...
    | otherwise = do
        ...
        return True
  where
    quantity = (gClient^.gcPers.cpInventory) UV.! (gItem^.giIndex)
    deathmatchValue = fmap (^.cvValue) deathmatchCVar
    ...
    applyInstantItem True = do
        when (isQuadItem (gItem^.giUse)) $ do
...
dropGeneralF edictRef gItemRef = do
    void (dropItem edictRef gItemRef)
    edict <- readRef edictRef
    gItem <- readRef gItemRef
...
proceedDropGeneral edictRef gItem (Just gClientRef) = do
    modifyRef gClientRef (\v -> v & gcPers.cpInventory.ix (gItem^.giIndex) -~ 1)
    GameUtil.validateSelectedItem edictRef
...
useSilencerF edictRef gItemRef = do
    edict <- readRef edictRef
    gItem <- readRef gItemRef
...
proceedUseSilencer edictRef gItem (Just gClientRef) = do
    modifyRef gClientRef (\v -> v & gcPers.cpInventory.ix (gItem^.giIndex) -~ 1)
    GameUtil.validateSelectedItem edictRef
...
useInvulnerability =
    ItemUse "useInvulnerability" (useGenericItem errMsg calcFrameNum updateFrameNum soundItem)
  where
    errMsg = "GameItems.proceedUseInvulnerability GClientRef is Nothing"
...
useBreather :: ItemUse
useBreather =
    ItemUse "useBreather" (useGenericItem errMsg calcFrameNum updateFrameNum soundItem)
  where
    errMsg = "GameItems.useBreather GClientRef is Nothing"
    calcFrameNum gClient frameNum
...
useEnviroSuit =
    ItemUse "useEnviroSuit" (useGenericItem errMsg calcFrameNum updateFrameNum soundItem)
  where
    errMsg = "GameItems.useEnviroSuit GClientRef is Nothing"
    calcFrameNum gClient frameNum
...
useGenericItem errMsg calcFrameNum updateFrameNum soundItem edictRef gItemRef = do
    edict <- readRef edictRef
    gItem <- readRef gItemRef
...
proceedUseGenericItem _ calcFrameNum updateFrameNum soundItem edictRef gItem (Just gClientRef) = do
    modifyRef gClientRef (\v -> v & gcPers.cpInventory.ix (gItem^.giIndex) -~ 1)
    GameUtil.validateSelectedItem edictRef
    gClient <- readRef gClientRef
...
pickupAncientHeadF edictRef otherRef = do
...
pickupAdrenalineF edictRef otherRef = do
    dmv <- fmap (^.cvValue) deathmatchCVar
    updateMaxHealth dmv
...
pickupBandolierF = pickupGeneric gItemError checkAmmoAndStuff
  where
    gClientError = "GameItems.pickupBandolier GClientRef is Nothing"
...
pickupPackF = pickupGeneric gItemError checkAmmoAndStuff
  where
    gClientError = "GameItems.pickupPack GClientRef is Nothing"
    gItemError = "GameItems.pickupPack GItemRef is Nothing"
    checkAmmoAndStuff Nothing = Com.fatalError gClientError
    checkAmmoAndStuff (Just gClientRef) = do
        checkCommonAmmoAndStuff checkMaxAmmo gClientRef
...
pickupGeneric gItemError checkAmmoAndStuff edictRef otherRef = do
    other <- readRef otherRef
...
checkCommonAmmoAndStuff checkMaxAmmo gClientRef = do
    checkMaxAmmo gClientRef
...
pickupKeyF edictRef otherRef = do
    edict <- readRef edictRef
    other <- readRef otherRef
...
pickupKeyCoop edict gClientRef gClient gItem
    ...
    | otherwise = do
        ...
        return True
  where
    isPowerCubeKey = (edict^.eClassName) == "key_power_cube"
...
pickupHealthF edictRef otherRef = do
    edict <- readRef edictRef
    other <- readRef otherRef
...
proceedPickupHealth edictRef edict otherRef other
    ...
    | otherwise = do
        ...
        return True
  where
    noNeedInHealth = noHealthIgnoreMax && alreadyAtFull
    ...
    resetToMaxHealth v | v > (other^.eMaxHealth) = other^.eMaxHealth
                       | otherwise = v
    checkTimedHealth
        | (edict^.eStyle) .&. Constants.healthTimed /= 0 = do
            levelTime <- use (gameBaseGlobals.gbLevel.llTime)
...
armorIndex edictRef = do
  ...
  where
    getArmorIndex Nothing = return 0
    getArmorIndex (Just gClientRef) = do
        gClient <- readRef gClientRef
...
pickArmorIndex gClient jacketArmor combatArmor bodyArmor
    ...
    | otherwise = 0
  where
    inventory = gClient^.gcPers.cpInventory

addAmmo :: Ref EdictT -> Ref GItemT -> Int -> Quake Bool
addAmmo edictRef gItemRef count = do
    edict <- readRef edictRef
    addPlayerAmmo (edict^.eClient)
  where
    addPlayerAmmo Nothing = return False
    addPlayerAmmo (Just gClientRef) = do
        gClient <- readRef gClientRef
        gItem <- readRef gItemRef
    ...
    applyPlayerAmmo gClientRef gClient gItem maxAmmo
        ...
        | otherwise = do
            modifyRef gClientRef (\v -> v & gcPers.cpInventory.ix (gItem^.giIndex) %~ updateAmmo maxAmmo)
            return True
    updateAmmo maxAmmo v = min (v + count) maxAmmo

dropItem :: Ref EdictT -> Ref GItemT -> Quake (Ref EdictT)
dropItem edictRef gItemRef = do
...
calcNoClientForward droppedRef edict = do
    modifyRef droppedRef (\v -> v & eEntityState.esOrigin .~ (edict^.eEntityState.esOrigin))
    return forward
  where
    (forward, _, _) = Math3D.angleVectors (edict^.eEntityState.esAngles) True False False

calcClientForward :: Ref EdictT -> Ref EdictT -> EdictT -> Ref GClientT -> Quake (V3 Float)
calcClientForward droppedRef edictRef edict gClientRef = do
    gClient <- readRef gClientRef
...
playItemSound edictRef soundItem gameImport = do
    soundIdx <- soundIndex (Just soundItem)
...
respawnItem _ edictRef (Just gItemRef) = do
    gItem <- readRef gItemRef
    setRespawn edictRef (fromIntegral (gItem^.giQuantity))
...
doRespawnF edictRef = do
    ...
    return False
  where
    pickRandomTeamMember edict _ =  do
        count <- countTeamDepth (edict^.eTeamMaster) 0
    ...
    countTeamDepth (Just entRef) count = do
        ent <- readRef entRef
    ...
    chooseError = do
        Com.fatalError "GameItems.doRespawn ent^.eTeamMaster/ent^.eChain is Nothing"
        return (Ref (-1))
...
dropTempTouchF edictRef otherRef plane surf = do
    edict <- readRef edictRef
...
dropMakeTouchableF edictRef = do
...
spItemHealthSmall edictRef = do
    deathmatch <- fmap (^.cvValue) deathmatchCVar
    dmflags <- fmap (truncate . (^.cvValue)) dmFlagsCVar :: Quake Int
    doSpawnItemHealthSmall deathmatch dmflags
  where
    doSpawnItemHealthSmall deathmatch dmflags
        | deathmatch /= 0 && (dmflags .&. Constants.dfNoHealth) /= 0 =
            GameUtil.freeEdict edictRef 
