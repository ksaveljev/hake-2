...
addCommand name cmd = do
    varStr <- CVar.variableString name
...
newAlias name cmd =
    globals.gCmdAlias %= (newlyCreatedAlias <|)
  where
    newlyCreatedAlias = newCmdAliasT & caName .~ name
                                     & caValue .~ cmd
...
restOfCommandLine idx count accum
  ...
  where
    updateAccum arg | idx == count - 1 = accum <> BB.byteString arg
...
execF = XCommandT "Cmd.execF" $
  ...
  where
    ...
    execScript fileName contents = do
        Com.printf (B.concat ["execing ", fileName, "\n"])
        CBuf.insertText contents

echoF :: XCommandT
echoF = XCommandT "Cmd.echoF" $ error "Cmd.echoF" -- TODO

cmdlist :: XCommandT
cmdlist = XCommandT "Cmd.cmdlist" $ error "Cmd.cmdlist" -- TODO
...
handleExistingAlias alias = do
    globals.gAliasCount += 1
...
sameAliasNameAs expected alias = sameNameAs expected aliasName
  where
    aliasName = BC.map toUpper (alias^.caName)
...
tokenize text idx
  ...
  where
    !newIdx = skipWhitesToEOL text idx
...
processVar text cmdArgc updatedIdx var = do
    when (cmdArgc < Constants.maxStringTokens) (updateArgv cmdArgc var)
    tokenize text updatedIdx
             
updateArgv :: Int -> B.ByteString -> Quake ()
updateArgv cmdArgc var = do
...
macroExpandString text len
  ...
  where
    lengthExceeded = len >= Constants.maxStringChars
    lineExceededError = do
        Com.printf (B.concat ["Line exceeded ", encode Constants.maxStringChars, " chars, discarded.\n"])
        return Nothing
...
processParsedVar text newInQuote newLen count idx newIdx var =
  ...
  where
    tryToExpand token
        ...
        | otherwise = do
            let newText = B.concat [B.take idx text, token, B.drop newIdx text]
            expand newText newInQuote (B.length newText) (count + 1) idx
    lineExceeded token = newLen + B.length token >= Constants.maxStringChars
    lineExceededError = do
        Com.printf (B.concat ["Expanded line exceeded ", encode Constants.maxStringChars, " chars, discarded.\n"])
        return Nothing
    loopDetected = count + 1 == 100
    loopDetectedError = do
...
writeMessage cmd = do
    MSG.writeByteI message stringCmd
    SZ.printSB message cmd
    c <- argc
    when (c > 1) addArgs
  where
    stringCmd = fromIntegral Constants.clcStringCmd
