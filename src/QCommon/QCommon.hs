...
initialCVars =
  ...
  where
    version = ("version", v, Constants.cvarServerInfo .|. Constants.cvarNoSet)
...
processUserCommands =
    CBuf.addLateCommands >>= process -- add + commands from command line
  where
    process True = SCR.endLoadingPlaque -- the user asked for something explicit so drop the loading plaque
    process False = runDefault -- the user didn't give any commands, run default action

runDefault :: Quake ()
runDefault =
    getDefaultCommand >>= CBuf.addText >> CBuf.execute
  where
...
handleLogStats logStats
    ...
    | otherwise = do
        CVar.update (logStats & cvModified .~ False)
        closeLogStatsFile
...
prepareLogStatsFile h = do -- IMPROVE: handle exceptions
    io $ do
...
showTrace :: Quake ()
showTrace = do
    stv <- fmap (^.cvValue) showTraceCVar
    when (stv /= 0) $ do
...
printTimeStats _ _ _ = do -- to avoid unused var warning
-- printTimeStats timeBefore timeBetween timeAfter =
    hsv <- fmap (^.cvValue) hostSpeedsCVar
    when (hsv /= 0) $
