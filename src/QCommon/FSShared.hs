...
fOpenFile fileName =
  ...
  where
    fromFileLink netPath True = fromDirectoryTree netPath
    fromFileLink _ False = return Nothing
    fromPackFile pack entry True = io $ do
        h <- openBinaryFile (BC.unpack (pack^.pFilename)) ReadMode
    ...
    fromDirectoryTree netPath = do
        h <- io (openBinaryFile (BC.unpack netPath) ReadMode)
        return (Just h)
