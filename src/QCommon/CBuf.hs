...
initialize :: Quake ()
initialize = SZ.initialize (globals.gCmdText) B.empty 8192
...
execute = do
  ...
  where
    ...
    finishExecution text = do
        updateSizeBuf (globals.gCmdText) 0 B.empty
        Cmd.executeString text
        globals.gCmdWait .= False
    execCommand text curSize idx = do
    ...
    updateSizeBuf sbLens curSize text =
        sbLens %= (\v -> v & sbCurSize .~ curSize
                           & sbData .~ text)

addLateCommands :: Quake Bool
addLateCommands = do
    args <- fmap (V.drop 1) (use (comGlobals.cgComArgv))
    addCommands args
  where
    addCommands args
        | any (not . B.null) args = checkArgumentCommands args
        | otherwise = return False
    checkArgumentCommands args = do
        let text = B.intercalate " " (V.toList args)
            commands = findCommands text 0 B.empty
