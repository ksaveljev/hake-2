...
config multiplayer
    | multiplayer = openIP
    | otherwise = do
        checkIPSocketClient =<< use (netGlobals.ngIpSocketClient)
        checkIPSocketServer =<< use (netGlobals.ngIpSocketServer)
  where
    checkIPSocketClient Nothing = return ()
    checkIPSocketClient (Just ipSocketClient) = do
        io (Socket.close ipSocketClient) -- IMPROVE: catch exceptions
        netGlobals.ngIpSocketClient .= Nothing
    checkIPSocketServer Nothing = return ()
    checkIPSocketServer (Just ipSocketServer) = do
        io (Socket.close ipSocketServer) -- IMPROVE: catch exceptions
...
sleep msec = do
  ...
  where
    proceedSleep Nothing _ = return ()
    proceedSleep (Just _) dedicated
        | (dedicated^.cvValue) == 0 = return ()
        | otherwise = io netSleep
    netSleep = do
        print (B.concat ["sleeping ", encode msec, " msec"])
...
adrToString adr = B.concat [encode a, ".", encode b, ".", encode c, ".", encode d, ":", encode (adr^.naPort)]
  where
    hostAddress = fromMaybe 0 (adr^.naIP) -- IMPROVE: is 0 as default ok here?
...
stringToAdr str
    ...
    | otherwise = do
        parsedAddress <- io (parseAddress (head address))
        either addressError applyIpAndPort parsedAddress
  where
    strLow = BC.map toLower str
    address = BC.split ':' str
    addressError e = do
        Com.printf (encode e `B.append` "\n")
        return Nothing
    applyIpAndPort ip
        | length address == 2 =
...
parseAddress hostname =
    handle (\(e :: IOException) -> return (Left e)) $ do
        resolved <- NBSD.getHostByName (BC.unpack hostname)
...
getPacket sock netFromLens netMessageLens = do
    done <- getLoopPacket fromLoopbackLens netFromLens netMessageLens
    getNetworkPacket netFromLens netMessageLens done fromIpSocketLens
  where
    fromLoopbackLens :: Lens' QuakeState LoopbackT
    fromLoopbackLens
...
getNetworkPacket netFromLens netMessageLens False socketLens = do
    socket <- use socketLens
    maybe (return False) readNetworkPacket socket
  where
    readNetworkPacket socket = do
        packet <- io (Socket.recvFrom socket 2048)
        maybe (return False) networkPacket packet
    networkPacket (buf, host, port) = do
    ...
    checkBuffer buf netMsg
        | B.length buf > (netMsg^.sbMaxSize) = do
            adr <- use netFromLens
            Com.printf (B.concat ["Oversize packet from ", adrToString adr, "\n"])
            return False
        | otherwise = do
            netMessageLens.sbCurSize .= B.length buf
...
doSendPacket buf adr (Just sock)
  ...
  where
    addressIPError = error "NET.doSendPacket adr^.naIP is Nothing"
    doSend ip = void (io (Socket.sendTo sock buf ip (adr^.naPort)))
...
doSendLoopPacket buf len socketLens = do
    loop <- use socketLens
    socketLens.lSend += 1
