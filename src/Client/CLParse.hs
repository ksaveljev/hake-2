...
parseServerMessage =
  do parseMessage =<< use (globals.gNetMessage)
     CLView.addNetGraph
     join (liftA2 checkSaveDemo demoRecording demoWaiting)
  where demoRecording = use (globals.gCls.csDemoRecording)
...
proceedParseMessage cmd
  ...
  | otherwise =
      do checkShowNet =<< clShowNetCVar
         doParseMessage cmd
         parseMessage =<< use (globals.gNetMessage)
  where checkShowNet showNetVar
...
showNet str showNetVar
  | (showNetVar^.cvValue) >= 2 =
      do readCount <- use (globals.gNetMessage.sbReadCount)
         Com.printf (B.concat [encode (readCount - 1), ":", str, "\n"])
  | otherwise = return ()

reconnect :: Quake ()
reconnect =
  do Com.printf "Server disconnected, reconnecting\n"
  ...
  where checkDownload Nothing = return ()
        checkDownload (Just downloadFile) =
          do Lib.fClose downloadFile
             globals.gCls.csDownload .= Nothing

parsePrintPacket :: Quake ()
parsePrintPacket =
  do i <- MSG.readByte (globals.gNetMessage)
     when (i == Constants.printChat) $
       do S.startLocalSound "misc/talk.wav"
...
parseServerData =
  ...
  where saveGameDir gameDir gameDirVar
        ...
        playCinematicOrLoadLevel playerNum levelName
          ...
          | otherwise =
              do Com.printf (B.concat ["Levelname: ", levelName, "\n"])
...
parseConfigString =
  do i <- MSG.readShort (globals.gNetMessage)
...
doParseConfigString i str refreshPrepped (Just oldStr)
  ...
  | i >= Constants.csSounds && i < Constants.csSounds + Constants.maxSounds =
      when refreshPrepped $
        do soundRef <- S.registerSound str
  ...
  | otherwise = return ()
  where rendererError = Com.fatalError "CLParse.doParseConfigString renderer is Nothing"
        parseModelConfigString renderer =
          do modelRef <- (renderer^.rRefExport.reRegisterModel) str
             globals.gCl.csModelDraw.ix (i - Constants.csModels) .= modelRef
             setModelClip
        setModelClip
          | B.take 1 str == "*" =
              do cModelRef <- CM.inlineModel str
                 globals.gCl.csModelClip.ix (i - Constants.csModels) .= Just cModelRef
          | otherwise =
              globals.gCl.csModelClip.ix (i - Constants.csModels) .= Nothing
        parseImageConfigString renderer =
          do imageRef <- (renderer^.rRefExport.reRegisterPic) str
             globals.gCl.csImagePrecache.ix (i - Constants.csImages) .= imageRef

parseStartSoundPacket :: Quake ()
parseStartSoundPacket = error "CLParse.parseStartSoundPacket" -- TODO

parseBaseline :: Quake ()
parseBaseline =
  do (newNum, bits) <- CLEnts.parseEntityBits [0]
