...
beginWhenReady clientStatic developer
    | ready = do
        checkCinematicTime =<< use (globals.gCl.csCinematicTime)
        updateScreen
        setDisableScreen =<< Timer.milliseconds
        setDisableServerCount =<< use (globals.gCl.csServerCount)
    | otherwise = return ()
  where
    ready = (clientStatic^.csDisableScreen) == 0 &&
            (developer^.cvValue) == 0 &&
...
timeRefreshF = XCommandT "SCR.timeRefreshF" $ do
...
renderFrame renderer c
  ...
  where
    noPageFlipRender idx = do
        globals.gCl.csRefDef.rdViewAngles._y .= idx / 128.0 * 360.0
        (renderer^.rRefExport.reRenderFrame) =<< use (globals.gCl.csRefDef)
    pageFlipRender idx = do
        globals.gCl.csRefDef.rdViewAngles._y .= idx / 128.0 * 360.0
        (renderer^.rRefExport.reBeginFrame) 0
        (renderer^.rRefExport.reRenderFrame) =<< use (globals.gCl.csRefDef)
        renderer^.rRefExport.reEndFrame
...
printTime start stop =
  ...
  where
    time = fromIntegral (stop - start) / 1000 :: Float
...
skyF :: XCommandT
skyF = XCommandT "SCR.skyF" (sky =<< Cmd.argc)

sky :: Int -> Quake ()
sky c
    | c < 2 = Com.printf "Usage: sky <basename> <rotate> <axis x y z>\n"
    | otherwise = do
        rotate <- if c > 2 then fmap Lib.atof (Cmd.argv 2) else return 0
        axis <- if c == 6
        ...
        v <- Cmd.argv 1
        renderer <- use (globals.gRenderer)
        setSky v rotate axis renderer
  where
    setSky v rotate axis renderer = (renderer^.rRefExport.reSetSky) v rotate axis
...
updateScreen2 = do
    ...
    when needToUpdate $ do
        checkStereoSeparation =<< clStereoSeparationCVar
        (numFrames, separation) <- checkStereo =<< clStereoCVar
        renderer <- use (globals.gRenderer)
        mapM_ (runFrames renderer separation) [0..numFrames-1]
        (renderer^.rRefExport.reEndFrame)
...
checkStereo stereo
    | (stereo^.cvValue) /= 0 = do
        stereoSeparation <- clStereoSeparationCVar
...
runFrames renderer separation idx = do
    (renderer^.rRefExport.reBeginFrame) separationValue
  ...
  where
    separationValue | idx == 0 = separation^._1
    ...
    checkStatLayout (Just statLayout) = do
        when (statLayout .&. 1 /= 0) drawLayout
...
doRunCinematic cl cls
    ...
    | frame > (cl^.csCinematicFrame) = do
        when (frame > (cl^.csCinematicFrame) + 1) $ do
            Com.printf (B.concat ["Dropped frame: ", encode frame, " > ", encode ((cl^.csCinematicTime) + 1)])
            globals.gCl.csCinematicTime .= (cls^.csRealTime) - (cl^.csCinematicFrame) * 1000 `div` 14
    ...
    | otherwise = return ()
  where
    frameF = fromIntegral (cls^.csRealTime) - fromIntegral (cl^.csCinematicTime) * 14 / 1000 :: Float
    frame = truncate frameF :: Int
    copyValue a b = a >>= (b .=)

checkFinishCinematic :: Maybe (UV.Vector Word8) -> Quake ()
checkFinishCinematic (Just _) = return ()
checkFinishCinematic Nothing = do
    stopCinematic
    finishCinematic
...
doRunConsole conLines conCurrent frameTime conSpeed
    ...
    | otherwise = return ()
  where
    v1 = conCurrent - (conSpeed^.cvValue) * frameTime
...
calcVrect :: Quake ()
calcVrect = do
    checkViewSize =<< viewSizeCVar
    join (liftA2 calcVrectAndSave viewSizeCVar vidDef)
  where
    vidDef = use (globals.gVidDef)

checkViewSize :: CVarT -> Quake ()
checkViewSize viewSize = do
    when ((viewSize^.cvValue) < 40) (void (CVar.set "viewsize" "40"))
...
proceedTileClear conCurrent dirty oldDirty vidDef =
    unless ((clear^.y2) <= (clear^.y1)) $ do
        ...
        renderer <- use (globals.gRenderer)
        void (startClear vrect renderer)
  where 
...
clearAbove renderer top clear
    | (clear^.y1) < top = do
        (renderer^.rRefExport.reDrawTileClear) (clear^.x1) (clear^.y1) ((clear^.x2) - (clear^.x1) + 1) (idx - (clear^.y1) + 1) "backtile"
        return (clear & y1 .~ top)
    | otherwise = return clear
  where
    idx | (clear^.y2) < (top - 1) = clear^.y2
        | otherwise = top - 1

clearBelow :: Renderer -> Int -> DirtyT -> Quake DirtyT
clearBelow renderer bottom clear
    | (clear^.y2) > bottom = do
        (renderer^.rRefExport.reDrawTileClear) (clear^.x1) idx ((clear^.x2) - (clear^.x1) + 1) ((clear^.y2) - idx + 1) "backtile"
        return (clear & y2 .~ bottom)
    | otherwise = return clear
  where
    idx | (clear^.y1) > bottom + 1 = clear^.y1
        | otherwise = bottom + 1

clearLeft :: Renderer -> Int -> DirtyT -> Quake DirtyT
clearLeft renderer left clear
    | (clear^.x1) < left = do
        (renderer^.rRefExport.reDrawTileClear) (clear^.x1) (clear^.y1) (idx - (clear^.x1) + 1) ((clear^.y2) - (clear^.y1) + 1) "backtile"
        return (clear & x1 .~ left)
    | otherwise = return clear
  where
    idx | (clear^.x2) < left - 1 = clear^.x2
        | otherwise = left - 1

clearRight :: Renderer -> Int -> DirtyT -> Quake DirtyT
clearRight renderer right clear
    | (clear^.x2) > right = do
        (renderer^.rRefExport.reDrawTileClear) idx (clear^.y1) ((clear^.x2) - idx + 1) ((clear^.y2) - (clear^.y1) + 1) "backtile"
        return (clear & x2 .~ right)
    | otherwise = return clear
  where
    idx | (clear^.x1) > right + 1 = clear^.x1
        | otherwise = right + 1

drawStats :: Quake ()
drawStats = do
    statsStr <- preuse (globals.gCl.csConfigStrings.ix Constants.csStatusBar)
    maybe statsStrError executeLayoutString statsStr
  where
    statsStrError = Com.fatalError "SCR.drawStats statsStr is Nothing"
...
drawCenterString = do
    ...
    unless (B.null str) $ do
        ...
        vidDef <- use (globals.gVidDef)
        renderer <- use (globals.gRenderer)
        drawLines str vidDef 9999 (calcY centerLines vidDef) 0 renderer
  where
    calcY centerLines vidDef
        | centerLines <= 4 = truncate (fromIntegral (vidDef^.vdHeight) * 0.35 :: Float)
        | otherwise = 48

drawLines :: B.ByteString -> VidDefT -> Int -> Int -> Int -> Renderer -> Quake ()
drawLines str vidDef remaining y start renderer = do
    addDirtyPoint x y
  ...
  where
    len = scanLineWidth str start 0 40
...
drawFPS fps
    ...
    | fps^.cvModified = do
        CVar.update (fps & cvModified .~ False)
...
doDrawFPS vidDef fpsValue = drawFPSByChar x 0 fpsValue
  where
    x = (vidDef^.vdWidth) - 8 * (B.length fpsValue) - 2

checkTimeDiff :: CVarT -> Int -> Int -> Quake ()
checkTimeDiff fps realTime lastTime
    | diff > truncate ((fps^.cvValue) * 1000) = do
    ...
    | otherwise = return ()
  where
    diff = realTime - lastTime

updateFrameStats :: Int -> Int -> Int -> Int -> Quake ()
updateFrameStats diff realTime frameCount lastFrames = do
    scrGlobals.scrLastFrames .= frameCount
    scrGlobals.scrLastTime .= realTime
    scrGlobals.scrFPSValue .= fpsStr
  where
    fpsValue = fromIntegral (frameCount - lastFrames) * 100000 / fromIntegral diff / 100.0 :: Float
...
drawPause :: Quake ()
drawPause = do
    showPause <- scrShowPauseCVar
    paused <- pausedCVar
    unless ((showPause^.cvValue) == 0 || (paused^.cvValue) == 0) $ do
        vidDef <- use (globals.gVidDef)
        renderer <- use (globals.gRenderer)
        dim <- (renderer^.rRefExport.reDrawGetPicSize) "pause"
...
drawConsole :: Quake ()
drawConsole = do
    Console.checkResize =<< use (globals.gVidDef)
    state <- use (globals.gCls.csState)
...
drawLoading renderer = do
  ...
  where
    dimError = Com.fatalError "SCR.drawLoading dim is Nothing"
...
parseLayoutString str x y width idx
  ...
  where
    parseWithToken newIdx token = do
        (x', y', width', finalIdx) <- processToken str x y width newIdx token
...
processStatStringToken stats configStrings x y width newIdx (Just token) = do
...
processIfToken str stats x y width newIdx (Just token)
    | value == 0 = do
        finalIdx <- skipToEndIf str newIdx
...
processStringToken x y width newIdx (Just token) = do
...
processString2Token x y width newIdx (Just token) = do
...
processPicToken x y width stats newIdx token =
  ...
  where
    proceedProcessPicToken value = do
        when (fromIntegral value >= Constants.maxImages) $
...
processClientToken width vidDef newIdx6 (Just token1) (Just token2) (Just token3) (Just token4) (Just token5) (Just token6) = do
    addDirtyPoint x' y'
  ...
  where
    x' = (vidDef^.vdWidth) `div` 2 - 160 + Lib.atoi token1
    ...
    proceedProcessClientToken clientInfo = do
...
processCtfToken width vidDef newIdx5 (Just token1) (Just token2) (Just token3) (Just token4) (Just token5) = do
    addDirtyPoint x' y'
    addDirtyPoint (x' + 159) (y' + 31)
    clientInfo <- use (globals.gCl.csClientInfo)
    when (clientInfoIdx >= Constants.maxClients || clientInfoIdx < 0) $
  ...
  where
    ...
    proceedProcessCtfToken clientInfo = do
...
processPicnToken :: Int -> Int -> Int -> Maybe B.ByteString -> Int -> Renderer -> Quake (Int, Int, Int, Int)
processPicnToken x y width (Just token) newIdx renderer = do
    addDirtyPoint x y
    addDirtyPoint (x + 23) (y + 23)
...
processNumToken x y newIdx2 (Just token1) (Just token2) = do
    stats <- use (globals.gCl.csFrame.fPlayerState.psStats)
    maybe statsError doDrawStat (stats UV.!? statsIdx)
  where
    width = Lib.atoi token1
...
processHnumToken x y idx serverFrame health flashes = do
    when (flashes .&. 1 /= 0) $ do
        renderer <- use (globals.gRenderer)
...
processingError msg = do
    Com.fatalError msg
    return (0, 0, 0, 0)

readNextFrame :: Quake (Maybe (UV.Vector Word8))
readNextFrame = do
    cinematicFileHandle <- use (globals.gCl.csCinematicFile)
    maybe cinematicFileHandleError proceedReadNextFrame cinematicFileHandle
  where
    cinematicFileHandleError = do
        Com.fatalError "SCR.readNextFrame cinematicFileHandle is Nothing"
        return Nothing
    proceedReadNextFrame cinematicFileHandle = do
        command <- io (BL.hGet cinematicFileHandle 4)
...
readSounds cinematicFileHandle cinematicFrame cin = do
  ...
  where
    start = cinematicFrame * (cin^.cSRate) `div` 14
...
stopCinematic = doStopCinematic =<< use (scrGlobals.scrCin.cRestartSound)
  where
    doStopCinematic False = return ()
    doStopCinematic True = do
        globals.gCl.csCinematicTime .= 0
...
doPlayVideo cinematicFileHandle = do
    endLoadingPlaque
    globals.gCls.csState .= Constants.caActive
    cinematicData <- io (BL.hGet cinematicFileHandle 20)
  ...
  where
    getCinematicData = (,,,,) <$> getInt <*> getInt <*> getInt <*> getInt <*> getInt

debugGraph :: Float -> Int -> Quake ()
debugGraph _ _ = io (putStrLn "SCR.debugGraph IMPLEMENT ME!")
...
huff1TableInit = do
  ...
  where
    cinematicFileHandleError = Com.fatalError "SCR.huff1TableInit cinematicFileHandle is Nothing"
    proceedTableInit cinematicFileHandle = do
        allCounts <- io (B.hGet cinematicFileHandle (256 * 256))
        let (hNodes, numHNodes) = calcNodes allCounts
        scrGlobals.scrCin.cHNodes1 .= Just hNodes
        scrGlobals.scrCin.cNumHNodes1 .= numHNodes
    calcNodes allCounts = runST $ do
        hNodes <- MUV.replicate (256 * 256 * 2) 0
...
tableInit hNodes allCounts idx = do
    hCount <- UV.unsafeThaw (UV.generate 512 (\i -> if i < 256 then fromIntegral (counts `B.index` i) else 0))
    hUsed <- MUV.replicate 512 0
    initNodes hNodes hUsed hCount (idx * 256 * 2) 256
  where
    counts = B.take 256 (B.drop (idx * 256) allCounts)
...
huff1Decompress frame size = do
  ...
  where
    ...
    index = (-256) * 2
    hNodesError = do
        Com.fatalError "SCR.huff1Decompress hNodes is Nothing"
        return UV.empty
    proceedHuff1Decompress numHNodes hNodes = runST $ do
        out <- MUV.new count
...
decompress frame hNodes numHNodes nodeNum inIdx outIdx out index count
    ...
    | otherwise = do
        (index', count', nodeNum', outIdx') <- processByte hNodes numHNodes nodeNum (frame `B.index` inIdx) outIdx out index count 0 8
...
processByte hNodes numHNodes nodeNum inByte outIdx out index count idx maxIdx
    ...
    | nodeNum < 256 = do
        ...
        case count' == 0 of
            ...
            False -> do
                let nodeNum' = hNodes UV.! (index' + (numHNodes UV.! nodeNum) * 2 + fromIntegral (inByte .&. 1))
...
drawField x y color width value
    ...
    | otherwise = do
        ...
        renderer <- use (globals.gRenderer)
        mapM_ (doDrawField renderer num color x' y) [0..len-1]
  where
    w = min width 5
    num = encode value
