...
gunNextF :: XCommandT
gunNextF = XCommandT "V.gunNextF" $ do
    globals.gGunFrame += 1
    gunFrame <- use (globals.gGunFrame)
    Com.printf (B.concat ["frame ", encode gunFrame, "\n"])

gunPrevF :: XCommandT
gunPrevF = XCommandT "V.gunPrevF" $ do
    globals.gGunFrame %= \v -> max 0 (v - 1)
    gunFrame <- use $ globals.gGunFrame
    Com.printf (B.concat ["frame ", encode gunFrame, "\n"])
  ...
  where 
    setGunModel c
        ...
        | otherwise = do
            ...
            registerModel arg renderer

registerModel :: B.ByteString -> Renderer -> Quake ()
registerModel name renderer = do
    model <- (renderer^.rRefExport.reRegisterModel) modelName
    globals.gGunModel .= model
  where
    modelName = B.concat ["models/", name, "/tris.md2"]
...
doRenderView stereoSeparation frameValid forceRefDef paused
    | frameValid && (forceRefDef || (paused^.cvValue) == 0) = do
    ...
    | otherwise = return ()
  where
    checkNonZeroVar f var = when ((var^.cvValue) /= 0) f
    checkZeroVar f var = when ((var^.cvValue) == 0) f
    copyValue aLens bLens = do
        v <- use aLens
        bLens .= v

checkStereoSeparation :: Float -> Quake ()
checkStereoSeparation stereoSeparation
    | stereoSeparation /= 0 = do
