...
addNetGraph = join (liftA2 proceedAddNetGraph scrDebugGraphCVar scrTimeGraphCVar)
  where
    proceedAddNetGraph debugGraph timeGraph
...
prepRefresh :: Quake ()
prepRefresh = do
    str <- preuse (globals.gCl.csConfigStrings.ix (Constants.csModels + 1))
    renderer <- use (globals.gRenderer)
    proceedPrepRefresh str renderer

proceedPrepRefresh :: Maybe B.ByteString -> Renderer -> Quake ()
proceedPrepRefresh Nothing _ = Com.fatalError "CLView.prepRefresh str is Nothing"
proceedPrepRefresh (Just str) renderer =
    unless (B.length str == 0) $ do
  ...
  where
    mapName = B.drop 5 (B.take (B.length str - 4) str) -- skip "maps/" and cut off ".bsp"
