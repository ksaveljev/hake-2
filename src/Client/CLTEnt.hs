...
doAddBeam beam modLightning org len pitch yaw d dist modelLength
    | (beam^.bModel) == modLightning && d <= modelLength = do
        r <- Lib.rand
        ClientV.addEntity (newEntityT & eOrigin .~ beam^.bEnd
                                      & eModel .~ beam^.bModel
...
constructBeams beam org len modLightning pitch yaw d dist modelLength
  ...
  where
    constructBeam r
        | (beam^.bModel) == modLightning =
            ClientV.addEntity (newEntityT & eOrigin .~ org
                                          & eModel .~ (beam^.bModel)
                                          & enFlags .~ Constants.rfFullBright
        ...
        | (explosion^.eType) == exPoly =
            if f >= (explosion^.eFrames) - 1
                then explosion & eType .~ exFree
                    ...
                    in explosion & eEnt.eAlpha .~ alpha
                                 & eEnt.eSkinNum .~ skinNum
                                 & eEnt.enFlags %~ (.|. flagsUpdate)
        | (explosion^.eType) == exPoly2 =
            if f >= (explosion^.eFrames) - 1
                then explosion & eType .~ exFree
                else explosion & eEnt.eAlpha .~ (5.0 - fromIntegral f) / 5.0
                               & eEnt.eSkinNum .~ 0
                               & eEnt.enFlags %~ (.|. Constants.rfTranslucent)
        | otherwise = explosion
