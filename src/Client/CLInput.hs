...
impulse :: XCommandT
impulse = XCommandT "CLInput.impulse" $ do
    arg <- Cmd.argv 1
...
doSendCmd cmdRef state
    ...
    | otherwise = do
        sendUserUpdate =<< use (globals.gUserInfoModified)
...
writeCommandsToBuf outgoingSequence = do
  ...
  where
    i = (outgoingSequence - 2) .&. (Constants.cmdBackup - 1)
...
calcFrameMsec sysFrameTime oldSysFrameTime
    ...
    | otherwise = diff
  where
    diff = fromIntegral sysFrameTime - oldSysFrameTime

updateOldSysFrameTime :: Quake ()
updateOldSysFrameTime = do
    sysFrameTime <- use (globals.gSysFrameTime)
...
baseMove cmdRef = do
  ...
  where
    resetCmdRef = do
        angles <- use (globals.gCl.csViewAngles)
        writeRef cmdRef (newUserCmdT & ucAngles .~ fmap truncate angles)
    checkInStrafe inStrafe sideSpeed
        | (inStrafe^.kbState) .&. 1 /= 0 = do
        ...
        | otherwise = return ()
    updateSideMove sideSpeed = do
        moveRight <- keyState (clientGlobals.cgInMoveRight)
    ...
    updateUpMove upSpeed = do
        up <- keyState (clientGlobals.cgInUp)
    ...
    checkForwardMove inKLook forwardSpeed
        | (inKLook^.kbState) .&. 1 == 0 = do
            forward <- keyState (clientGlobals.cgInForward)
...
finishMove cmdRef = do
  ...
  where
    updateViewAngles viewAngles =
        modifyRef cmdRef (\v -> v & ucAngles .~ fmap Math3D.angleToShort viewAngles)
    updateImpulse inImpulse = do
        modifyRef cmdRef (\v -> v & ucImpulse .~ fromIntegral inImpulse)
...
calcMs cmdRef frameTime =
  ...
  where
    ms = truncate (frameTime * 1000) :: Int
    ms' | ms > 250 = 100
        | otherwise = ms
...
keyState keyLens = do
  ...
  where
    calcMsec key
        | (key^.kbState) /= 0 = do
            sysFrameTime <- fmap fromIntegral (use (globals.gSysFrameTime))
    ...
    calcResult msec frameMsec
        ...
        | otherwise = val
      where
        val = fromIntegral msec / fromIntegral frameMsec
...
clampPitch :: Quake ()
clampPitch = do
    pitch <- use (globals.gCl.csFrame.fPlayerState.psPMoveState.pmsDeltaAngles._x)
