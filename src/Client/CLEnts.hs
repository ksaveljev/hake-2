...
parseEntityBits bits = readA >>= readB >>= readC >>= readD >>= readNumber
  where readA = MSG.readByte (globals.gNetMessage)
        readB a | a .&. Constants.uMoreBits1 /= 0 =
                    do b <- MSG.readByte (globals.gNetMessage)
                       return (a .|. (b `shiftL` 8))
                | otherwise = return a
        readC b | b .&. Constants.uMoreBits2 /= 0 =
                    do c <- MSG.readByte (globals.gNetMessage)
                       return (b .|. (c `shiftL` 16))
                | otherwise = return b
        readD c | c .&. Constants.uMoreBits3 /= 0 =
                    do d <- MSG.readByte (globals.gNetMessage)
                       return (c .|. (d `shiftL` 24))
                | otherwise = return c
        readNumber d | d .&. Constants.uNumber16 /= 0 =
                         do num <- MSG.readShort (globals.gNetMessage)
                            return (num, d : tail bits)
                     | otherwise =
                         do num <- MSG.readByte (globals.gNetMessage)
                            return (num, d : tail bits)

parseDelta :: EntityStateT -> Traversal' QuakeState EntityStateT -> Int -> Int -> Quake ()
parseDelta from to number bits =
  do modelIndex <- getModelIndex
