...
setUpGun gunModelRef ps ops cl = do
    ...
    gunRef <- io $ newIORef (newEntityT & eModel .~ Just gunModelRef
                                        & eAngles .~ angles
                                        & eOrigin .~ origin
                                        & eFrame .~ frame
                                        & eOldOrigin .~ origin
...
tweakBeamsColor cl s1 ent renderfx
  ...
  where
    ...
    doCheckDisguise skin@(Just skinRef) model renderer = do
...
checkColorShells ent effects renderfx = do
    when (effects .&. Constants.efColorShell /= 0) $ do
        ...
        entRef <- io $ newIORef $ ent & enFlags .~ renderfx'' .|. Constants.rfTranslucent
                                      & eAlpha .~ 0.3
        ClientV.addEntity entRef
...
checkModelIndex2 s1 ent
    | (s1^.esModelIndex2) /= 0 = do
        ...
        entRef <- io (newIORef (buildEnt model configString))
        ClientV.addEntity entRef
