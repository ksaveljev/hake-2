...
menuAddItem menuFrameworkRef menuItemRef = do
  ...
  where
    checkEmptyMenu menu
        ...
        | otherwise = return ()
    addItem menu
        | (menu^.mfNItems) < Constants.maxMenuItems = do
...
menuCenter menuFrameworkRef = do
  ...
  where
    getMenuItemHeight (MenuListRef ref) = fmap (^.mlGeneric.mcY) (readRef ref)
...
menuMultiplayerF = XCommandT "Menu.menuMultiplayerF" $ do
    multiplayerMenuInit
...
menuKeysF :: XCommandT
menuKeysF = XCommandT "Menu.menuKeysF" $ do
    keysMenuInit
    pushMenu keysMenuDrawF keysMenuKeyF

menuDownloadOptionsF :: XCommandT
menuDownloadOptionsF = XCommandT "Menu.menuDownloadOptionsF" $ do
...
menuNames = V.fromList
    [ "m_main_game", "m_main_multiplayer", "m_main_options"
    , "m_main_video" , "m_main_quit"
    ]

mainDrawF :: XCommandT
mainDrawF = XCommandT "Menu.mainDrawF" $ do
    vidDef <- use (globals.gVidDef)
    renderer <- use (globals.gRenderer)
    mainDraw vidDef renderer

mainDraw :: VidDefT -> Renderer -> Quake ()
mainDraw vidDef renderer = do
    widest <- V.foldM findWidest (-1) menuNames
  ...
  where
    findWidest widest menuName = do
        dim <- (renderer^.rRefExport.reDrawGetPicSize) menuName
...
drawPlaque renderer xOffset yStart = do
    dim <- (renderer^.rRefExport.reDrawGetPicSize) "m_main_plaque"
    maybe dimError doDrawPlaque dim
  where
    dimError = Com.fatalError "Menu.drawPlaque dim is Nothing"
    doDrawPlaque (w, h) = do
...
gameMenuDrawF = XCommandT "Menu.gameMenuDrawF" $ do
...
optionsMenuDrawF = XCommandT "Menu.optionsMenuDrawF" $ do
...
loadGameMenuInit = do
  ...
  where
    setupLoadGameMenuAction saveStrings idx actionRef = do
...
loadGameMenuDrawF = XCommandT "Menu.loadGameMenuDrawF" $ do
...
loadGameMenuKey key
    | key == KeyConstants.kEscape || key == KeyConstants.kEnter = do
...
saveGameMenuInit = do
  ...
  where
    setupSaveGameMenuAction saveStrings idx actionRef = do
...
saveGameMenuDrawF = XCommandT "Menu.saveGameMenuDrawF" $ do
...
saveGameMenuKey key
    | key == KeyConstants.kEnter || key == KeyConstants.kEscape = do
...
joinServerMenuInit = do
    ...
    searchLocalGames
  where
    setupJoinServerAction idx actionRef = do
...
joinServerMenuDrawF = XCommandT "Menu.joinServerMenuDrawF" $ do
...
addressBookMenuInit = do
  ...
  where
    setupAddressBookMenuAction idx fieldRef = do
        adr <- getAddress idx
    ...
    getAddress idx = do
        adr <- CVar.get ("adr" `B.append` encode idx) "" Constants.cvarArchive
        maybe adrError return adr
    adrError = do
        Com.fatalError "Menu.addressBookMenuInit adr is Nothing"
        return (CVarT B.empty B.empty Nothing 0 False 0)

addressBookMenuDrawF :: XCommandT
addressBookMenuDrawF = XCommandT "Menu.addressBookMenuDrawF" $ do
...
setAddressBookCVars idx maxIdx
  ...
  where
    fieldRef = addressBookFields V.! idx
...
loadMapNames = do
  ...
  where
    mapNamesError = Com.comError Constants.errDrop

applyMapNames :: B.ByteString -> Quake ()
applyMapNames buffer = do
    when (numMaps == 0) $
...
readMapsList Nothing = do
    buffer <- FS.loadFile "maps.lst"
    maybe (return (Left "couldn't find maps.lst\n")) (return . Right) buffer
readMapsList (Just h) = do
    buffer <- io (B.hGetContents h) -- IMPROVE: exception handling
    return (Right buffer)
...
multiplayerMenuDrawF = XCommandT "Menu.multiplayerMenuDrawF" $ do
...
quitDrawF :: XCommandT
quitDrawF = XCommandT "Menu.quitDrawF" $ do
    renderer <- use (globals.gRenderer)
    vidDef <- use (globals.gVidDef)
    quitDraw vidDef renderer
  where
    quitDraw vidDef renderer = do
        dim <- (renderer^.rRefExport.reDrawGetPicSize) "quit"
...
keysMenuDrawF = XCommandT "Menu.keysMenuDrawF" $ do
...
banner name = do
    renderer <- use (globals.gRenderer)
    vidDef <- use (globals.gVidDef)
    dim <- (renderer^.rRefExport.reDrawGetPicSize) name
    maybe dimError (drawPic vidDef renderer) dim
  where
...
forceMenuOff :: Quake ()
forceMenuOff = do
    menuGlobals %= (\v -> v & mgDrawFunc .~ Nothing
                            & mgKeyFunc .~ Nothing
                            & mgMenuDepth .~ 0)
...
proceedAddToServerList adr info numServers localServerNames
    ...
    | otherwise = do
        modifyRef (joinServerActions V.! numServers) (\v -> v & maGeneric.mcName .~ Just info)
...
creditsKey key = do
    when (key == Constants.kEscape) popMenu
...
consoleFunc = do
  ...
  where
    proceedConsoleFunc True = CBuf.addText "killserver\n"
    proceedConsoleFunc False = do
        Key.clearTyping
        Console.clearNotify
        forceMenuOff
        globals.gCls.csKeyDest .= Constants.keyConsole
...
findSaveGame gameDir idx = do
    havePermissions <- io (FS.canRead nameStr)
    updateSaveGameEntry havePermissions
  where
    name = B.concat [gameDir, "/save/save", encode idx, "/server.ssv"]
    nameStr = BC.unpack name
    updateSaveGameEntry True = do
        qf <- QuakeFile.open name
...
searchLocalGames = do
    menuGlobals.mgLocalServerNames .= V.replicate Constants.maxLocalServers Constants.noServerString
    renderer <- use (globals.gRenderer)
    drawTextBox renderer 8 (120 - 48) 36 3
...
joinServerFunc actionRef = do
    action <- readRef actionRef
    localServerNames <- use (menuGlobals.mgLocalServerNames)
    numServers <- use (menuGlobals.mgNumServers)
    when (correctServer (action^.maGeneric.mcN) localServerNames numServers) $ do
        localServerNetAdr <- use (menuGlobals.mgLocalServerNetAdr)
        CBuf.addText (B.concat ["connect ", NET.adrToString (localServerNetAdr V.! (action^.maGeneric.mcN)), "\n"])
        forceMenuOff
  where
    correctServer index localServerNames numServers =
...
startServerActionFunc = do
    mapNames <- use (menuGlobals.mgMapNames)
...
menuPrint renderer cx cy str = drawString 0 (B.length str)
  where
    drawString idx maxIdx
        | idx >= maxIdx = return ()
        | otherwise = do
            drawCharacter renderer (cx + 8 * idx) cy (ord (BC.index str idx) + 128)
            drawString (idx + 1) maxIdx

drawCharacter :: Renderer -> Int -> Int -> Int -> Quake ()
drawCharacter renderer cx cy num = do
    vidDef <- use (globals.gVidDef)
